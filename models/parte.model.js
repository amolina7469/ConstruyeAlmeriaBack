...
const getDailyById = (dailyId) => {
  return db.query('select date, daily.id, users.name as username, users.surname as usersurname, works.address, daily.km, works.pricekm, clients.name, clients.surname,containers.type as container, vehicles.brand, vehicles.registration, vehicles.model, vehicles.description, group_concat(distinct tool_id) as tools, group_concat(distinct worker_id) as workers from daily inner join works on work_id = works.id inner join clients on client_id = clients.id  inner join vehicles on vehicle_id = vehicles.id inner join daily_has_tools on daily.id = daily_has_tools.daily_id inner join daily_has_workers on daily.id = daily_has_workers.daily_id inner join containers on container_id = containers.id inner join users on user_id = users.id where daily.id = ? group by id; ', [dailyId]);
}
